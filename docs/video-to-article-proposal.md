...
Prepared 23 September 2026. Updated to reflect Eduard's standing instruction to complete and publish every video article from start to finish after full checks. The native hourly heartbeat **Publish new video articles** (automation ID `publish-new-video-articles`) is **ACTIVE** in this task; tool creation and saved `automation.toml` were verified. The Kosovo–Germany pilot is published from release `db62113`; Railway deployment and both live language pages passed verification. Direct Google submission remains pending account sign-in.
...
Use compact, visible footnote styling for sources and calculations (`.article-notes`), keeping the main article's reading size unchanged. The selected operating direction is a local Codex account workflow with the requested GPT-6 Astra (`gpt-6-astra`) and High reasoning (`high`), without a separate OpenAI API key. The native task-message tool accepted these settings for the initial workflow follow-up. Heartbeats expose no model fields, and persistent model inheritance for later runs has not been verified; do not guarantee Astra High for every scheduled run. See `docs/video-article-workflow.md` for source access, the active hourly workflow, publication checks, and supported Google submission steps.

The pilot is `src/content/writing/{en,sq}/kosovo-germany-grocery-prices.md`, with four responsive charts in both languages, a product price table, linked primary sources, and downloadable calculation data. Edit those Markdown files directly while preserving existing user changes. Update numeric inputs in `scripts/data/kosovo-price-comparison.json` and regenerate chart assets with `uv run scripts/generate-kosovo-price-charts.py`. Keep the prose and displayed numbers aligned when inputs change. Its status is **published and live-verified**: [English](https://eduardluta.com/writing/kosovo-germany-grocery-prices/) and [Albanian](https://eduardluta.com/sq/writing/kosovo-germany-grocery-prices/). Canonical, language, schema, sitemap, RSS, asset, and indexability checks passed. The video fallback is usable; embedded playback is not yet verified.
...
| Scheduling | Native hourly heartbeat **Publish new video articles** (`publish-new-video-articles`) is ACTIVE in this task; creation and saved configuration verified | Catch up on the next available scheduled execution; immediate app-open triggering is unverified |
...
1. **Pilot article — publishing in progress:** release `db62113` is pushed and Railway is building the completed bilingual article and assets. Verify deployment, live EN/SQ URLs, and indexability before marking it published.
...
3. **Automation — hourly heartbeat ACTIVE:** **Publish new video articles** runs in this task under the standing full-publishing instruction, with durable catch-up and duplicate guards. Each actual run must finish validation, publication, and live verification before reporting success; scheduling activation alone is not a successful content run. Immediate app-open triggering and persistent heartbeat model inheritance remain unverified. Refine the voice using Eduard's later edits without overwriting them.
