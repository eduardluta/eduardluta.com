...
Prepared 23 September 2026. Eduard has given standing authorization to finish and publish video articles from start to finish after the checks below pass. No recurring discovery or publishing schedule has been activated; authorization to publish is not evidence that a recurring job is running.
...
Requested model: **GPT-6 Astra (`gpt-6-astra`), High reasoning (`high`)**. Select these in the task's model picker / Advanced controls before enabling a recurring run in this task. A saved instruction naming a model cannot change the runtime model by itself. The current running turn has not been switched by writing this document. A standalone scheduled task, if Eduard later chooses one, should explicitly save those two settings.

Keep the computer on, the desktop app running, and this project available. Do not promise work while the computer is asleep or the app is closed. Eduard requires missed work to catch up when the app is available again. The workflow must resume from durable state on its next available execution after reopening; native immediate app-open triggering is still being verified and must not be represented as guaranteed or enabled.
...
The intended starting cadence is an hourly native check of the public `@eduardluta` TikTok profile while the desktop workflow can run, subject to confirmed scheduling support and activation. Eduard can also paste a published video link into this task. No recurring check is active yet.
...
Track the platform video ID and article slug so repeated checks, deployment retries, and cross-posts cannot create duplicate articles. Inspect both local content and the live site before creating or publishing an entry. The existing pilot is being published:

- TikTok ID: `7688677111879732501`
- Slug: `kosovo-germany-grocery-prices`
- Status: publishing in progress; the bilingual article is complete locally. Mark it published only after deployment and live URL/indexability checks succeed.
...
An hourly native recurring run is the intended starting cadence if supported and activated. Catch-up logic is required regardless of whether the runtime can fire immediately on app opening or waits until the next available scheduled execution. No recurrence or immediate app-open trigger is active merely because this document describes it; record the actual supported/activated behavior after tool confirmation.
...
4. Include the original durable thumbnail, the platform video, useful charts and factual visuals, descriptive alt text, and meaningful captions. Use readable desktop/mobile chart layouts. Put source and calculation notes in the compact, visible `.article-notes` section.
